...
var Roundware = {
    ...
    //     // listen to a stream
    //     $('#listen').click(function(){ Roundware.listen(); });

    //     // modify stream parameters
    //     $("#modify-stream").click(function(){ Roundware.modify_stream(); });

    //     // show/hide the geocoding GUI based on whether the include_lat_lng checkbox
    //     // is checked (show) or not (hide).
    //     $('#include_lat_lng').click(function(){
    //         $('#mapblock').toggle();
    //     });

    //     // add geo-coding handler to mapaddress block
    //     $('#mapaddress').autogeocomplete( Roundware.geoconfig );
    ...
    //         dataType: 'json',
    //         success: function(data) {
    //             $('#listen-form').append(Roundware.parse_tags(data.listen));
    //         },
    //         error: function(data) {
    ...
    //     var url = Roundware.url + '?operation=' + operation + '&session_id=' + Roundware.session_id + '&tags=' + l;
    //     if (lat && lng && $('#include_lat_lng:checked').length)
    //     {
    ...
    //         success: function(data) {
    //             var flashvars = {};
    //             var params = {}; // object-embedding parameters, i.e. translated to <param name="value" />
    //             var attributes = {};
    //             swfobject.embedSWF("rw/wimpy_button.swf?theFile=" + data.stream_url + "&autoplay=yes&loopMe=yes", "wimpy", 30, 30, "10.1.0", "", flashvars, params, attributes);
    ...
    //         dataType: 'json',
    //         success: function(data) {
    //             console.log('stream modified; new url: ' + url);
    //         },
    //         error: function(data) {
    ...
    listen_main: function (project_id) {
            ...
            $("#jquery_jplayer_1").jPlayer({
                ready: function () {
                    $(this).jPlayer("setMedia", {
                        mp3: "http://scapesaudio.dyndns.org:8000/stream4567.mp3"
                    });
                },
        ...
        $("#update-stream").click(function () {
            Roundware.show_spinner('Updating the stream...', 2);
    ...
    listen_form: function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                // $('.full-float-block').eq(0).prepend('<h2>2. Filter your choices with the options below and then click Modify Stream.</h2>');
    ...
    get_url: function (operation) {
        ...
        // l += "," + Roundware.siteTags;  // add siteTags for selected sites to list; comment out if no Sites in project
        //
        // var lat = $('#latitude').val();
        // var lng = $('#longitude').val();
    ...
    modify_stream: function () {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                //console.log('stream modified; new url: ' + Roundware.url);
            },
            error: function (data) {
                console.log('stream modify failure');
            }
    ...
    // // UPLOAD FUNCTIONS
    // //
    // upload_main : function(project_id)
    // {
    //     // init
    //     Roundware.init(project_id, Roundware.upload_form);

    //     // auto-complete handler for the address field of the map
    //     $('#mapaddress').autogeocomplete( Roundware.geoconfig );

    //     // save button handler pulls envelope data into form
    //     $('#save_button').click(function(){ return Roundware.upload_save_pressed(); });
    ...
    //         dataType: 'json',
    //         success: function(data) {
    //             $('#recording').append(Roundware.parse_tags(data.speak));
    //         },
    //         error: function(data) {
    //             console.log('could not retrieve tags to generate a form');
    //         }
    ...
    // /**
    //  * Generate an RW envelope via AJAX calls, and refactor the questions form fields
    //  * as tags, stash it all in the form and carry on!
    //  */
    // upload_save_pressed : function()
    // {
    //     Roundware.create_envelope();
    //     Roundware.set_tags();
    //     console.log('session_id is ' + Roundware.session_id);
